 ...
 *                                  lcd_bvh.c                                      *
 *                                                                                 *
 *                        writed by TRUNG KIEN PHAM 8/6/2024                         *
 *                                 16ETE - DUT                                     *
 *                                                                                 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
{  
      ...
      HAL_Delay(10);	// 4-bit mode
	    lcd_write_command(CLEAR_DISPLAY);	
			HAL_Delay(10);	// Clear screen
	    lcd_write_command(DISPLAY_ON_OFF_CONTROL | OPT_D);
			HAL_Delay(10);	// Lcd-on, cursor-off, no-blink
    	lcd_write_command(ENTRY_MODE_SET | OPT_INC);	
			HAL_Delay(10);	// Increment cursor
}
/**/
